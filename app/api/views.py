...
class MetricaCalculadora:
    @staticmethod
    def calculate_metrica(queryset):
        return queryset.count() * 10 + 5

class MisturaCalculadora:
    def get_metrica(self, queryset):
        return MetricaCalculadora.calculate_metrica(queryset)

class VerdurasViewSet(viewsets.ModelViewSet, MisturaCalculadora):
...
class EntregaViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        entrega_info = [{'nome_cliente': entrega.nome_do_consumidor, 'endereco': entrega.localidade_da_entrega} for entrega in self.queryset]
